...
class UDPHandler(socketserver.BaseRequestHandler):   
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            ...
            print(origin," calling ",destination) #hodit do .txt

            f = open("dennik.txt", "a")
            f.write(origin + " calling " + destination + "\n")
            f.close()
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                f = open("dennik.txt", "a")
                f.write("480 Temporarily Unavailable" + "\n\n")
                f.close()
        else:
    ...
    def processRequest(self):
        ...
        if len(self.data) > 0:
            ...
            if rx_register.search(request_uri):
                ...
                self.sendResponse("200 0K toto je 1")
            elif rx_publish.search(request_uri):
                self.sendResponse("200 0K toto je 2")
            elif rx_notify.search(request_uri):
                self.sendResponse("200 0K toto je 3")
